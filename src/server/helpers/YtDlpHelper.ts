...
import { CACHE_PATH, CACHE_FILE_PREFIX, COOKIES_FILE, DOWNLOAD_PATH } from '@/server/constants';
...
export class YtDlpHelper {
  ...
  constructor(querys: {
    ...
    this.videoInfo.sponsorBlock = querys.sponsorBlock || false;
    this.videoInfo.sponsorBlockCategories = querys.sponsorBlockCategories || '';
    this.videoInfo.audioOnly = querys.audioOnly || false;
    this.videoInfo.splitChapters = querys.splitChapters || false;
    this.videoInfo.concurrentFragments = querys.concurrentFragments || 1;
    this.videoInfo.embedMetadata = querys.embedMetadata || false;
    this.videoInfo.audioOnly = querys.audioOnly || false;
    this.videoInfo.splitChapters = querys.splitChapters || false;
    this.videoInfo.concurrentFragments = querys.concurrentFragments || 1;
    ...
    switch (metadata.type) {
      case 'video': {
        ...
        if (metadata?.isLive) {
          ...
          if (this.videoInfo?.sponsorBlock && this.videoInfo.sponsorBlockCategories) {
            options.push('--sponsorblock-remove', this.videoInfo.sponsorBlockCategories);
          }
        ...
        if (videoInfo.file.path) {
          (async () => {
            try {
              ...
              const ffmpegThumb = new FFmpegHelper({ filePath: videoInfo.file.path! });
              const ok = await ffmpegThumb.generateThumbnail(thumbPath);
              if (ok) {
                videoInfo.localThumbnail = thumbFile;
                videoInfo.updatedAt = Date.now();
                await CacheHelper.set(uuid, videoInfo);
              }
            } catch {}
